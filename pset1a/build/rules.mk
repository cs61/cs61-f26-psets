...
# hhtest.cc hhtest-tinyalloc.cc hhtest-smallalloc.cc hhtest-largealloc.cc
PSETFILES = README.md AUTHORS.md .gitignore GNUmakefile check.pl build/rules.mk \
	m61.hh m61.cc nastytest1.cc hexdump.cc hexdump.hh \
	$(sort $(wildcard test*.cc))
GRADEFILES = grade/entergrade.sh
LAB = 1
-include ../common/devrules.mk

remake-sol:
	$(call run,$(MAKE_DEPSOL))
.PHONY: remake-sol
clean: remake-sol

gradedir: distdir
	rm -rf pset$(LAB)grade.tgz
	rm -rf pset$(LAB)/m61.cc pset$(LAB)/m61.hh pset$(LAB)/README.md pset$(LAB)/AUTHORS.md pset$(LAB)/out
	echo rm:pset$(LAB)/out >.gitcheckout
	../common/mklab.pl LAB=$(LAB) SOL=20 pset$(LAB) $(GRADEFILES)
	mv pset$(LAB)/GNUmakefile pset$(LAB)/grade/GNUmakefile.dist
	mv pset$(LAB)/build/rules.mk pset$(LAB)/grade/rules.mk.dist
	(cd pset$(LAB); ls test*.cc) | sed 's/\.cc$$//' > pset$(LAB)/grade/testlist
gradeball pset$(LAB)grade.tgz: gradedir always
	rm -f pset$(LAB)grade.tgz
	tar -caf pset$(LAB)grade.tgz pset$(LAB) .gitcheckout
	rm -rf pset$(LAB) .gitcheckout

.PHONY: gradedir gradeball
#endif
